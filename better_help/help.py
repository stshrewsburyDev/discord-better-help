...
class Help(HelpCommand):
    def __init__(self, **options):
        ...
        self.timeout_delete = options.pop("timeout_delete", False)
        self.timeout_remove_controls = options.pop("timeout_remove_controls", False)
        ...
        self.closed_delete = options.pop("closed_delete", False)
        self.closed_remove_controls = options.pop("closed_remove_controls", False)
        self.closed_show_message = options.pop("closed_show_message", True)
    ...
    def get_ending_note(self):
        default = "Type {0}{1} [command] for more info on a command.\n"\
                  "You can also type {0}{1} [category] for more info on a category.".format(self.clean_prefix, self.invoked_with)
